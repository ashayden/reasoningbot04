...
class SynthesisExpert(BaseAgent):
    ...
    def synthesize(self, topic: str, research_results: List[ResearchResult]) -> Optional[str]:
        ...
        prompt = (
            "As an academic synthesis expert, create a comprehensive final report that balances "
            "scholarly precision with engaging clarity. Use concrete examples and specific evidence "
            "to support each point. Maintain an authoritative yet approachable voice throughout.\n\n"
            f"Topic: {topic}\n\n"
            "Research Findings:\n"
            + "\n\n---\n\n".join(summary_points)
